@extends('layouts.app', ['activePage' => 'profile', 'titlePage' => __('User Profile'), 'notification' => $notification])
          ...
          <form method="post" action="{{ route('profile.update') }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Ubah Profil') }}</h4>
                <p class="card-category">{{ __('Informasi User') }}</p>
              </div>
              <div class="card-body ">
                @if (session('status'))
                  <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('nama_user') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('nama_user') ? ' is-invalid' : '' }}" name="nama_user" id="input-name" type="text" placeholder="{{ __('Name') }}" value="{{ old('nama_user', auth()->user()->nama_user) }}" required="true" aria-required="true"/>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('nip') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('nip') ? ' is-invalid' : '' }}" name="nip" id="input-nip" type="text" placeholder="{{ __('NIP') }}" value="{{ old('nip', auth()->user()->nip) }}" required />
                      @if ($errors->has('nip'))
